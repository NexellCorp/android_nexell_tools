...
def write_header(output_file, d):
    ...
    # make partition: 4byte
    output_file.write(p32(d['make_partition']))
    # reserved: 4byte
    output_file.write(p32(0))
    ...
    for image in d['images']:
        ...
        # dest size: 8byte
        if 'dest_size' in image:
            output_file.write(p64(image['dest_size']))
        else:
            output_file.write(p64(0))
        # src offset
        if 'src_offest' in image:
            output_file.write(p64(image['src_offset']))
        else:
            output_file.write(p64(0))
        # src size
        if 'src_size' in image:
