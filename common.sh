...
function build_bl1_s5p4418()
{
    ...
    make clean
    if [ "${QUICKBOOT}" == "true" ]; then
        make CHIPNAME="${chip}" BOARD="${board}" KERNEL_VER="4" SYSLOG="n" DEVICE_PORT="${boot_device_port}" SUPPORT_USB_BOOT="n" SUPPORT_SDMMC_BOOT="y" QUICKBOOT=1 OTA_AB_UPDATE=n
    else
        make CHIPNAME="${chip}" BOARD="${board}" KERNEL_VER="4" SYSLOG="n" DEVICE_PORT="${boot_device_port}" SUPPORT_USB_BOOT="n" SUPPORT_SDMMC_BOOT="y" OTA_AB_UPDATE=n
    fi
...
function build_bl2_s5p4418()
{
    ...
    make clean
    if [ "${QUICKBOOT}" == "true" ]; then
        make QUICKBOOT=1 SUPPORT_OTA_AB_UPDATE=n
    else
        make SUPPORT_OTA_AB_UPDATE=n
    fi
    popd
...
function post_process()
{
    ...
    if [ "${BUILD_DIST}" == "true" ]; then
        ...
        cp ${TOP}/out/dist/${board_name}-target_files-eng.$(whoami).zip ${result_dir}/target_files.zip
